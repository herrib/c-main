...
static int minus1(int x) { return x - 1; }

static int concat_digits(int a, int b) { return b * 10 + a; }

static int times(int x, int y) { return x * y; }

static int times2(int x) { return x * 2; }

static int times30(int x) { return x * 30; }
...
static int big_if_three(int x) { return x < 3 ? 111 : 222; }
...
static void
test_compute_cells_update_value_when_dependencies_are_changed(void) {
  //TEST_IGNORE();
  struct reactor *r = create_reactor();
  struct cell *input = create_input_cell(r, 1);
  struct cell *output = create_compute1_cell(r, input, plus1);
...
test_callbacks_not_called_if_dependencies_change_but_output_value_doesnt_change(
  ...
  struct cell *plus_one = create_compute1_cell(r, input, plus1);
  struct cell *minus_one = create_compute1_cell(r, input, minus1);
  struct cell *always_two = create_compute2_cell(r, plus_one, minus_one, minus);
